...
function initializeDB() {
  ...
  if (DEBUG) console.log('Security database initialized');
}
...
export function resetToFirstLogin(employeeNumber) {
  const db = getDB();
  db.prepare(`
    UPDATE employee_security
    SET
...
export function getAllSecurityRecords() {
  const db = getDB();
  return db.prepare(`
    SELECT
      es.*,
      COUNT(ps.id) as push_count
    FROM employee_security es
    LEFT JOIN push_subscriptions ps ON ps.employee_number = es.employee_number
    GROUP BY es.employee_number
    ORDER BY
      CASE WHEN es.last_login IS NULL THEN 1 ELSE 0 END,
      es.last_login DESC
  `).all();
}
...
export function getAdminStats() {
  const db = getDB();
  const total = db.prepare('SELECT COUNT(*) as count FROM employee_security').get().count;
  const locked = db.prepare(
    `SELECT COUNT(*) as count FROM employee_security WHERE lockout_until > datetime('now')`
  ).get().count;
  const pushSubs = db.prepare(
    'SELECT COUNT(DISTINCT employee_number) as count FROM push_subscriptions'
  ).get().count;
  const recentLogins = db.prepare(
    `SELECT COUNT(*) as count FROM login_attempts WHERE success = 1 AND attempted_at > datetime('now', '-24 hours')`
  ).get().count;
  return { total, locked, pushSubs, recentLogins };
}

export function getRecentLoginAttempts(limit = 25) {
  const db = getDB();
  return db.prepare(
...
export function createResetToken(employeeNumber) {
  const db = getDB();
  const crypto = require('crypto');
  const token = crypto.randomBytes(32).toString('hex');
  // Clean up any old tokens for this employee first
  db.prepare('DELETE FROM reset_tokens WHERE employee_number = ?').run(employeeNumber.toString());
  db.prepare(`
    INSERT INTO reset_tokens (token, employee_number, expires_at)
    VALUES (?, ?, datetime('now', '+15 minutes'))
  `).run(token, employeeNumber.toString());
  return token;
}

export function validateResetToken(employeeNumber, token) {
  const db = getDB();
  ...
  `).get(token, employeeNumber.toString());
  if (!row) return false;
