    ...
    if (isINSWORKA) {
      ...
      result = await pool.query(`
        SELECT EMEMP, EMQEM, EMLNM, EMFNM, EMMNM, EMSTR, EMCIT, EMST, EMZP5,
               ...
               EMSTA, EMFW4, EMFDW, EMSTW, EMCRFD
        FROM ${table}
        WHERE TRIM(EMEMP) = ?
        FETCH FIRST 1 ROW ONLY
...
/**
 * Get pay stub earnings and deduction detail lines for a specific pay run
 * @param {string|number} employeeNumber
 * @param {string|number} runDate - HCRUN value (e.g. 20260001)
 * @returns {{ earnings: Array, deductions: Array }}
 */
export async function getPayStubDetail(employeeNumber, runDate) {
  try {
    const pool = await getConnection();
    const library = config.employeeLibrary;

    // Get employee hidden ID
    const empResult = await pool.query(
      `SELECT EMQEM FROM ${library}.${config.employeeTable} WHERE TRIM(EMEMP) = ? FETCH FIRST 1 ROW ONLY`,
      [employeeNumber.toString()]
    );
    if (empResult.length === 0) return { earnings: [], deductions: [] };
    const emqem = empResult[0].EMQEM;

    // Earnings lines
    const earningRows = await pool.query(`
      SELECT HSQSC, HSSAL, HSSHR
    ...
    for (const row of earningRows) {
      let description = `Code ${row.HSQSC}`;
      try {
        const sc = await pool.query(
          `SELECT SCTIT FROM ${library}.SALCODZZ WHERE SCQSC = ? FETCH FIRST 1 ROW ONLY`,
          [row.HSQSC]
        );
        if (sc.length > 0) description = (sc[0].SCTIT || '').trim();
      } catch { /* use code fallback */ }
      earnings.push({
        description,
        amount: row.HSSAL ?? 0,
        hours: row.HSSHR ?? 0
    ...
    for (const row of dedRows) {
      if ((row.HDDED ?? 0) === 0 && (row.HDMAT ?? 0) === 0) continue;
      let description = `Code ${row.HDQDC}`;
      try {
        const dc = await pool.query(
          `SELECT DCTIT FROM ${library}.DEDCODZZ WHERE DCQDC = ? FETCH FIRST 1 ROW ONLY`,
          [row.HDQDC]
        );
        if (dc.length > 0) description = (dc[0].DCTIT || '').trim();
      } catch { /* use code fallback */ }
      deductions.push({
        description,
        employeeAmount: row.HDDED ?? 0,
        employerAmount: row.HDMAT ?? 0
      });
    }

    return { earnings, deductions };
  } catch (error) {
    console.error('Error fetching pay stub detail:', error);
    throw new Error('Failed to fetch pay stub detail');
  }
}
